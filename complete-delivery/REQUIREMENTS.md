...
| CD-006 | CodeRabbit is an explicit configurable gate with trusted identity and no unresolved live findings. | `coderabbit=required`; trusted App/status/review matching in `delivery_gate.py`; all live threads remain blocking. | `test_delivery_gate.py` trusted signal and unresolved-thread cases. |
| CD-007 | CI and review failures route through a bounded fix/test/push/report loop. | `complete-delivery-pr-gate` expansion; maximum 12 attempts; ordered children. | `test_pack_contract.py::FormulaContractTests::test_external_review_loop_is_bounded_and_ordered` |
| CD-008 | Merge uses the protected PR path and recorded merge SHA, with no admin bypass. | merge prompt and `delivery-merged.sh`. | Prompt/command assertions in `test_pack_contract.py`. |
...
| CD-010 | One outcome-first HTML/CSS report updates throughout and reaches Live only after verification. | report milestone steps; `delivery_report.py`; `delivery-report-valid.sh`; safe publisher. | `test_delivery_report.py`; `test_publish_delivery_report.py`; formula milestone assertions. |
