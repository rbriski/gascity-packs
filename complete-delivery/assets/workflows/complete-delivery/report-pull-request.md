...
and make the next action "Resolve required CI and review findings." Mention
CodeRabbit only when the workflow explicitly configures it as `optional` or
`required`. Run `report_publish_command` with `DELIVERY_REPORT_DIR` when
configured.
