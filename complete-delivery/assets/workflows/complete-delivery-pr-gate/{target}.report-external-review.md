...
Before reading gate evidence, mutating the living report, or any provider
wait/poll, run `.gc/scripts/checks/delivery-external-review-deadline.sh --validate`.
Do not report or wait when it fails. Immediately before the
living-report mutation, run that validation again; failure writes blocker-only
evidence and closes non-pass without mutating the report.
...
When the snapshot is passing for `delivery.head_sha`, or the proven publication transition applies, name no blocker and set the immediate next action to the `external-review-loop` terminal mechanical check. Keep `external-review` active and never reuse the prior gate artifact as current-head authority before top-level `complete-delivery/report-green.md` publishes the protected-merge next action.

This pre-terminal lane may publish only configured active or blocked status;
it must never publish `external-review` as `passed` or a protected-merge next
action. Immediately before running `report_publish_command`, run
`.gc/scripts/checks/delivery-external-review-deadline.sh --validate` again.
Run `report_publish_command` with `DELIVERY_REPORT_DIR` when configured only
after that second validation. If the second validation or publication fails,
...
been recorded. If evidence is missing, malformed, stale, or root-head-mismatched
other than the exact proven publication transition above, close with a non-pass
