...
test_mayor_durable_formula_v2_contract() {
    ...
    grep -F 'Any known multi-step or multi-wave initiative that must continue without Mayor' "$prompt" >/dev/null ||
        fail "Mayor prompt must identify durable multi-step and multi-wave initiatives"
    grep -F 'babysitting must launch one configured Formula v2 workflow.' "$prompt" >/dev/null ||
        fail "Mayor prompt must require exactly one configured Formula v2 workflow"
